...
namespace Divan.Test
{
    ...
    public class CouchTest
    {
        ...
        public void ShouldGetProperJsonValueForQueryKey()
        {
            ...
            query.Key("a string");
//				Assert.That(query.Options["key"].Equals("\"a string\""), String.Format("value={0}", query.Options["key"]));
				Assert.That(query.Options["key"].Equals("a string"), String.Format("value={0}", query.Options["key"]));
				query.Key(12);
				Assert.That(query.Options["key"].Equals("12"), String.Format("value={0}", query.Options["key"]));
// this seems to be very dangerous 
// the automatic conversion depends on country and cultural settings 
// e.g. '.' will be converted to ',' with language=de,de
				//query.Key(-12.01);
				//Assert.That(query.Options["key"].Equals("-12.01"), String.Format("value={0}", query.Options["key"]));
            query.Key(true);
				Assert.That(query.Options["key"].Equals("True"), String.Format("value={0}", query.Options["key"]));
            query.Key(false);
				Assert.That(query.Options["key"].Equals("False"), String.Format("value={0}", query.Options["key"]));
            query.Key(null);
				Assert.That(query.Options["key"].Equals("null"), String.Format("value={0}", query.Options["key"]));
        ...
        /// </summary>
		  ...
		  {
			  var ecars = new List<ECar>();
			  ecars.Add(new ECar("shortDistance2", "town", false));
			  ecars.Add(new ECar("longDistance2", "weekendtrips", true));
			  db.SaveArbitraryDocuments<ECar>(ecars, true);
			  var ids = new List<String>() { "shortDistance2", "longDistance2" };
			  var ecars2 = db.GetArbitraryDocuments<ECar>(ids,() => new ECar());
			  var test = ecars2.ToList<ECar>();
			  Assert.That(test[0].name.Equals(ecars[0].name));
			  Assert.That(test[1].name.Equals(ecars[1].name));
		  }
